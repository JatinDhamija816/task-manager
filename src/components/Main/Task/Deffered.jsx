...
const Deffered = () => {
    const { data } = useData()
    const temp = data.filter((item) => item.status === 'deffered')
    return (
        <>
            <div className='bg-white ' >
                <h1 className='bg-red-400 flex justify-center p-1'>Deffered</h1>
                {
                    temp.map((task) => (
                        <div className='m-2 pb-4' key={task.id}>
                            <div className='w-5/6  bg-gray-300 m-auto mt-4 rounded-md'>
                                ...
                                <div className='p-3'>
                                    <p className='bg-blue-400 p-1 rounded-md text-center'>Deffered</p>
                                </div>
                            </div>
                        </div>
                    ))
